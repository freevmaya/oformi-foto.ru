...
var PJCONST = {
    ...
    TMPLURLPATHPREVIEW  : TMPLURL + 'preview120/',
...
var PJCanvas = new Class({
    ...
    setColors: function(color) {
        ...
        if (hole) {
            if (color) {
                ...
                if (!ct) ct = new colorTransform({adjustment: 1})
                ct.setOptions(Object.clone(color)); 
                hole.image.setFilter('colorTransform', ct);
            } else hole.image.setFilter('colorTransform', null); 
        }
    },
    ...
    refreshRect: function() {
        var coord = this.getParent().getSize();
        var rect = new Rectangle(0, 0, coord.x, coord.y);
