...
define('ITEMPERPAGE_MOBILE', 20 * 8);
...
//        print_r($task);
        ...
        $query = "SELECT SQL_CALC_FOUND_ROWS * FROM gpj_tmplOptions tmo WHERE tmo.`tmpl_id` < 60000 AND tmo.`active` = 1 ORDER BY tmo.`tmpl_id` DESC LIMIT $start,".$ITEMSPERPAGE;
                ...
                if (($groupIds[0] == 100000) || ($groupIds[0] == 'holiday')) {
                    ...
                    if (@$holiday_id = $groupIds[1]) {                                                                                                                                             
                        ...
                        if (!isset($task[3])) { // Если нет выбранной страницы
                            $date = date('Y-m-d');
                            if ($near_holiday = DB::line("SELECT * FROM `gpj_holiday` WHERE date>='{$date}' AND disabled=0 ORDER BY `date` LIMIT 0, 1")) { // Есть текущий или ближайший праздник
                            
                                $near_holiday_id = $near_holiday['holiday_id'];
                                $line_count = DB::line("SELECT COUNT(`holiday_id`) AS `count` FROM `gpj_holiday` WHERE date<'{$date}' AND disabled=0"); // Получаем количество праздников перед ним
