...
class appController extends controller {
    ...
    public function view() {
		...
		if ($tmpl = $this->getTemplate($tmpl_id)) {
			...
			$this->redirect($this->createController('article'), 'viewArticle');
			return;
		}
        }
    ...
    public function getTemplate($tmpl_id) {
        ...
        if (!is_numeric($tmpl_id)) {
            $tmpl_id = DB::one("SELECT tmpl_id FROM gpj_templates ORDER BY tmpl_id DESC LIMIT 1");
        }
