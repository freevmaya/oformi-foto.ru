<?
    ...
    class dataModel {
        ...
        protected function getList() {
            ...
            $searchText = $this->getVar('searchText', null);            
            ...
            if ($searchText) {
                $searchList = preg_split('/_+/' , $searchText);
                $searchWhere = '';
                foreach ($searchList as $word) {
                    $searchWhere .= ($searchWhere?' OR ':'')."(`name` LIKE '%{$word}%' OR `desc` LIKE '%{$word}%')"; 
                }
                ...
                $query = "SELECT SQL_CALC_FOUND_ROWS tmo.tmpl_id FROM `gpj_templates` tmpls INNER JOIN gpj_tmplOptions tmo ON tmpls.tmpl_id = tmo.tmpl_id 
                            WHERE tmpls.`tmpl_id` < 60000 AND tmo.`active`=1 AND $langWhere AND ($searchWhere) GROUP BY tmpls.`tmpl_id` DESC LIMIT $start,".$itemCount;
            } else if ($groups) {
            ...
            if ($totalPages > $page) {
                $nextURL = 'page='.($page + 1).'&count='.$itemCount;
                if ($searchText)
                    $nextURL .= '&searchText='.$searchText;
                else $nextURL .= '&groups='.$groups;
                $list[] = $nextURL; 
            }
            return $list;
        }
