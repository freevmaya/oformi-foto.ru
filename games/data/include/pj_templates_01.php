            ...
            if ($groups) {
                ...
                if ($count == 1) {
                    if ($groupIds[0] == NEWGROUPID) {
                        $query = "SELECT SQL_CALC_FOUND_ROWS id AS tmpl_id FROM gpj_templates_new GROUP BY id ORDER BY id DESC" ;
        ...
        protected function getCats_rus() {
            ...
            if (($lang == '') || ($lang == 'Rus')) {
                ...
                $query   = "SELECT DATE_FORMAT(`date`, '%d.%m') AS `date`, holiday_id + ".HOLIDAYGROUP." as group_id, `name` AS `name`, `name` AS `partName` FROM gpj_holiday ".
                        "WHERE `deftml_id`>0 AND `date`>='$minDate' AND `date`<='$maxDate' AND `type` < 10";
