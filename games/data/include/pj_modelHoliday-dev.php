<?
    ...
    class dataModel extends base_model {
        protected function getList() {
            ...
            $query      = "SELECT *, (SELECT COUNT(tmpl_id) FROM gpj_holidayTmpls WHERE holiday_id=h.holiday_id) AS tmpl_count  FROM gpj_holiday h WHERE h.`date`>='$minDate' AND h.`date`<='$maxDate' AND h.`type` < 10";
        ...
        public function allHolidays() {
            ...
            $query = "SELECT * FROM gpj_holiday h INNER JOIN gpj_holidayTmpls ht ON h.holiday_id = ht.holiday_id AND h.deftml_id=ht.tmpl_id WHERE h.`date`>='$startDate' AND h.`date`<'$endDate'";
            return DB::asArray($query);
        }
/*        
