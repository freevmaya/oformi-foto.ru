...
class ga_transaction {
    ...
    function begin($request) {
        if ((($request->getVar('service_id', -1) > 0) && ($request->getVar('service_id', 100) <= 10)) ||
            ...
            if ($transaction = query_line('SELECT * FROM ga_transaction WHERE transaction_id = '.$request->getVar('transaction_id', 0)))
                ...
                $query = "INSERT INTO ga_transaction (`transaction_id`, `user_id`, `service_id`, `sms_price`, `other_price`, `debug`) 
