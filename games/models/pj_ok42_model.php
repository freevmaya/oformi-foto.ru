...
//    ERROR---        
    ...
    class pj_ok42 extends g_model {
        
        private $intervals  = array(2001=>"-24 hour", 2008=>"-1 week", 2009=>"-1 month");
        
        private function sys_message() {
            return array('sys_message'=>'База данных входящих/исходящих открыток находиться на профилактике. Просим прощения за доставленные неудобства.');
        }
            ...
            if (count($users)) {
                ...
                if ($count) {
                    foreach ($users as $user) {
                        $query = "REPLACE INTO pjok_invite (`user_id`, `inviteUser`) VALUES ({$params[0]}, $user)";
            ...
            foreach ($list as $key=>$item) {
                if (($item['rate'] == 0) || ($item['votes'] == 0)) { // Отсеиваем все с нулевым рейтингом
                    ...
                    if ($vic['votes'] > $item['votes']) { // Это не победа
                        ...
                        DB::query("INSERT `pjok_gameVictory` VALUES ({$item['id']}, '{$item['name']}', $uid, '{$item['time']}', {$item['effects']}, {$item['votes']}, 0)");
...
//            $where = addWhere($where, '(b.banType is NULL)');
            $query = "SELECT g.id as id, g.name as name, g.uid as uid, g.rate as rate, g.`time` as `time`, g.`noVictory` as noVictory, v.votes as isMyVote, g.`debug` as `debug`, g.`effects` as effects,
        ...
        public function friendGameList($params) {
            ...
            $query = "SELECT g.id as id, g.name as name, g.uid as uid, g.rate as rate, g.`time` as `time`, g.`noVictory` as noVictory, g.`debug` as `debug`, g.`effects` as effects,
        ...
        public function getGame($params) {
            $line = DB::line('SELECT g.id as id, g.name as name, g.uid as uid, g.rate as rate, g.`time`, g.`noVictory` as noVictory, v.votes as isMyVote, g.`effects` as effects,
                            (SELECT COUNT(content_id) AS comment_count FROM pjok_comments WHERE content_id=g.id) as comment_count
        ...
        public function renameGame($params) {
            return array('result'=>DB::query("UPDATE `pjok_game` SET name='".mysql_escape_string(urldecode($params[1]))."' WHERE id={$params[0]}"));
        } 
...
//              Удалить файлы
        ...
        public function addGameCollage($params) {
            ...
            $result     = DB::query("INSERT INTO `pjok_game` (uid, `name`, rate, `effects`, time, debug) VALUES ({$params[0]}, '{$params[1]}|{$params[2]}', 0, '{$params[4]}', '".date('Y-m-d H:i:s')."', $debug)");
