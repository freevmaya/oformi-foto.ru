...
//            $this->setStatus(array($params[0], STATE_GAMEUSER));
        ...
        protected function uploadAsJPG($jpg, $id) {
            ...
            $rsize = explode('x', SHARESIZE);
            $img  = imagecreatefromjpeg(GAMEPATH.$file_name);
            $dest = imagecreatetruecolor($rsize[0], $rsize[1]);
            $scale = $rsize[0] / imagesx($img);
            
            imagecopyresampled($dest, $img, 0, 0, 0, 0, $rsize[0], $rsize[1], imagesx($img), $rsize[1] / $scale);
        ...
        public function addComment($params) {
            GLOBAL $mysqli;
            $params[2] = mysqli_escape_string($mysqli, urldecode($params[2]));  
            $content_id = $params[1];
