<?
    ...
    //include_once(INCLUDE_PATH.'/statistic_2.php');
...
//    ERROR---        
    ...
    class pjok_n25 extends g_model {
        ...
        public function sendCard($params) {
            ...
            $send_result = array();
            $send_result['state'] = DB::query("INSERT INTO pjok_send (`uid`, `sendTo`, `params`, `time`, `date`, `card_id`) 
                                                VALUES ('{$params[0]}', '{$params[1]}', '{$params[2]}', '".date('Y-m-d H:i:s')."', '".date('Y-m-d')."', {$cardInfo[0]})");
            $send_result['send_id'] = DB::one("SELECT LAST_INSERT_ID()");                                  
            
            if (!$sendTo = DB::line("SELECT `inCount` FROM pjok_options WHERE `uid`={$params[1]}")) $this->createNewUser($params[1], false);
            if ($params[4]) // Отмечаем что есть исходящие открытки
                DB::query("UPDATE pjok_options SET isOut = 1 WHERE `uid`={$params[0]}");
            
            // Отмечаем что есть входящая открытка
            DB::query("UPDATE pjok_options SET `inCount` = `inCount` + 1 WHERE `uid`={$params[1]}");
        ...
        public function createNewUser($uid, $createDateSet=true){
            ...
            DB::query("REPLACE pjok_options VALUES ({$uid}, 1, '{$createDate}', '{$createDate}', '{$createTime}', 0, 0, 0, '{$createDate}')");
            DB::query("REPLACE `pjok_refplace` (`uid`, `date`, `refplace`) VALUES ({$uid}, '{$createDate}', -1)");
...
//            $query = "SELECT o.*, i.birthday FROM `pjok_options` o LEFT JOIN `pjok_info` i ON i.uid={$params[0]} WHERE o.uid={$params[0]}";
            ...
            if (!$result) {                                                     // Создаем запись нового пользователя
                ...
                $result['alert']        = '';
                DB::query("UPDATE pjok_options SET createDate=CURDATE(), vidgetDate=CURDATE(), visitDate=CURDATE(), visitTime=CURTIME() WHERE `uid`={$params[0]}");
            } else {
                if (strtotime($result['vidgetDate']) < strtotime($date)) {
                    $result['start_today'] = 1;
                    DB::query("UPDATE pjok_options SET vidgetDate=CURDATE(), visitDate=CURDATE(), visitTime=CURTIME() WHERE `uid`={$params[0]}");
                } else DB::query("UPDATE pjok_options SET visitTime=CURTIME(), visitDate=CURDATE() WHERE `uid`={$params[0]}");
                
                //$saveRec = DB::line("SELECT `count` FROM pjok_save WHERE `uid`={$params[0]} AND `date`='$date'"); 
                //$result['saveCount'] = $saveRec['count'];
                $vipRecords = DB::line("SELECT * FROM pjok_transaction WHERE 
                ...
                if ($vipRecords) {
                    ...
                    DB::query("INSERT INTO pjok_transaction (`time`, `user_id`, `service_id`, `price`, `state`) VALUES (NOW(), {$params[0]}, 1, {$price}, 'r_may')");
                    DB::query("DELETE FROM pjok_transaction WHERE transaction_id={$vipRecords['transaction_id']} AND user_id={$params[0]} AND time='{$vipRecords['time']}'");
                    
                    $result['alert'] = CHANGEVIPSTATUS;
                    
                } else $result['alert'] = DB::line("SELECT a.alert_id, t.* FROM `pjok_alerts` a, `gpj_alertTemplates` t WHERE a.uid={$params[0]} AND a.alert_id=t.id AND t.type<100");
            }
            ...
            if ($isRef) {
                ...
                DB::query("REPLACE `pjok_refplace` (`uid`, `date`, `refplace`) VALUES ({$params[0]}, '$date', {$refCode})");
            }
        ...
        public function getInBox($params) {
            ...
            DB::query("UPDATE pjok_options SET `inCount`=".(count($result)).", `receiveCount`=".$receiveCount." WHERE `uid`={$params[0]}");
            return $result;
        }
...
/*        public function receiveCardViewed($params) {
            return array('result1'=>DB::query("UPDATE pjok_send SET `received`=1 WHERE `send_id`={$params[0]}"), 
                         'result2'=>DB::query("UPDATE pjok_options SET `receiveCount`=`receiveCount`+1 WHERE `uid`={$params[0]}"));
        }*/

        public function receiveCardViewedAsUser($params) {
            return array('result1'=>DB::query("UPDATE pjok_send SET `received`=1 WHERE `send_id`={$params[1]}"), 
                         'result2'=>DB::query("UPDATE pjok_options SET `receiveCount`=`receiveCount`+1 WHERE `uid`={$params[0]}"));
        }
        ...
        public function noPayCollector($uid, $count) {
            return array('result'=>DB::query("REPLACE pjok_save (uid, `date`, `count`) VALUES ('$uid', '".date('Y-m-d')."', $count)"));        
        }
        ...
        public function getRate($params) {
            ...
            $result['last_user'] = DB::line("SELECT uid, price FROM pjok_rateUserStop ORDER BY `date` DESC LIMIT 0, 1");
            $result['rate'] = DB::line("SELECT save_rate, user_rate FROM gpj_tmplOptions WHERE tmpl_id={$params[0]}");
            return $result;
        }
/*            
...
//            return array('result'=>'');
        ...
        public function uploadPNG($params) {
            ...
            $uid = $params[0];
            $file_name = USERDATAPATH.$params[1];
            ...
            return array('file'=>USERDATAURL.$params[1], 'time'=>time());
        }
...
//Transaction
        ...
        function getBalance($params) {
            ...
            $query = "SELECT SUM(`price`) as `balance`, SUM(case when transaction_id > 0 then 1 else 0 end) as `payCount`, COUNT(transaction_id) AS t_count FROM pjok_transaction WHERE (`service_id` IN (".implode(',', $services).")) AND `user_id`={$uid}";
            $result = DB::line($query);
...
//              Удалить файлы
        ...
        public function listComment($params) {
           return DB::asArray("SELECT DATE_FORMAT(c.`time`, '%d.%m.%y %H:%i') AS `time`, c.uid, c.comment, c.comment_id
        ...
        public function getSmailfir($params) {
            GLOBAL $mysqli;
            $uid = mysqli_escape_string($mysqli, $params[0]);
            $data = DB::line("SELECT * FROM pjok_smallfir WHERE uid={$uid}");
            if (@$data['disabled'] == 1) {
                return array('disabled'=>1);
            }  
            if (!$data) return array('create'=>1);
            return $data;
        }
        
        public function setSmailfir($params) {
            GLOBAL $mysqli;
            
            $response = array('result'=>0);
            $uid = mysqli_escape_string($mysqli, $params[0]);
            $json = mysqli_escape_string($mysqli, $params[1]);
            $response['result'] = DB::query("REPLACE pjok_smallfir (uid, `data`) VALUES ({$uid}, '$json')")?1:0;
            return $response;
        }    
        
        public function disabledSmailfir($params) {
            GLOBAL $mysqli;
            $response = array('result'=>0);
            $uid = mysqli_escape_string($mysqli, $params[0]);
            $disabled = mysqli_escape_string($mysqli, $params[1]);
            $response['result'] = DB::query("UPDATE pjok_smallfir SET `disabled`={$disabled} WHERE uid={$uid}")?1:0;
            return $response;
        }    
    }
?>
