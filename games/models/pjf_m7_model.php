...
/*            if (isset($params[5])) {
            ...
            if (!$sendTo = query_line("SELECT `inCount` FROM gpj_options WHERE `uid`={$params[1]}")) $this->createNewUser($params[1], false);
            ...
            sql_query("UPDATE gpj_options SET `inCount` = `inCount` + 1 WHERE `uid`={$params[1]}");
...
//            $result = $result && DB::query("REPLACE gpj_groups (`group_id`) VALUES %s", $values);
        ...
        public function applyGroupTemplate($params) {
            ...
            if (DB::line('SELECT * FROM gpj_tmplOptions WHERE tmpl_id='.$tmpl_id) === false)
                DB::query("INSERT INTO gpj_tmplOptions (`tmpl_id`, `autor_id`, `active`) VALUES ($tmpl_id, 0, 1)");
