<?
    ...
    class pjf03 extends g_model {
        ...
        public function getCard($params) {
            return query_line("SELECT * FROM gpj_send WHERE send_id=".@intval($params[1]));
        }
...
/*                $users = array();
        ...
        public function getSmailfir($params) {
            GLOBAL $mysqli;
            $data = null;
            if ($uid = mysqli_escape_string($mysqli, $params[0])) {
                $data = DB::line("SELECT * FROM of_smallfir WHERE uid={$uid}");
                if (@$data['disabled'] == 1) {
        ...
        public function setSmailfir($params) {
            ...
            if ($uid = mysqli_escape_string($mysqli, $params[0])) {
                ...
                $response['result'] = DB::query("REPLACE of_smallfir (uid, `data`) VALUES ({$uid}, '$json')")?1:0;
            }
            return $response;
        }    
        ...
        public function disabledSmailfir($params) {
            ...
            if ($uid = mysqli_escape_string($mysqli, $params[0])) {
                ...
                $response['result'] = DB::query("UPDATE of_smallfir SET `disabled`={$disabled} WHERE uid={$uid}")?1:0;
            }
            return $response;
        } 
    }
