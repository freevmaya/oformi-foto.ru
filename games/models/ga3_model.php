<?
    ...
    class ga3 extends g_model {
        ...
        function uploadJPGFrame($params) {
            GLOBAL $GLOBALS;
            
            $fileName   = 'f'.$params[0].'_'.$params[1];
            $jpgPath    = GA_TMPPATH.$fileName.'.jpg';
            $gifPath    = GA_TMPPATH.$fileName.'.gif';
            $jpg        =  $GLOBALS["HTTP_RAW_POST_DATA"];
            
            if (file_exists($jpgPath)) unlink($jpgPath);
            $file = fopen($jpgPath, 'w+');
            fwrite($file, $jpg);
            fclose($file);
            
            $result = 0;
            if ($jpgImage = imagecreatefromjpeg($jpgPath)) {
                $result = imagetruecolortopalette($jpgImage, true, 255);
                $result = imagegif($jpgImage, $gifPath) && $result;
                $result = imagedestroy($jpgImage) && $result;
            }
        ...
        function createGIF($params) {
            ...
            $filePath = RESULTRELATIVEPATH.$fileName;
...
/*            // Передаем GIF файл на сервер хранилище
            ...
            $fileURL = $result['response'][0]['fileURL'];*/
