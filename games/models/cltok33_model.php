...
//            $url = $a[1];
            ...
            return array('user'=>$rec, 
                        'pays'=>DB::asArray("SELECT * FROM `clt_oktransaction` WHERE `user_id` = %s AND (`time`>=".PAYSTARTDATE." OR `service_id` = 5 OR (`service_id` = 6 AND `time`>=".VIPSTARTDATE."))", $params),
                        'checkVictory'=>$this->checkVictory($params[0]),
                        'myVic'=>DB::asArray("SELECT * FROM `clt_okgameVictory` WHERE `uid`=%s", $params),
                        'isMyGame'=>DB::line("SELECT COUNT(id) as `count` FROM `clt_okgame` WHERE `uid`=%s", $params),
                        'message'=>'',
                        'plugins'=>array(
        ...
        protected function checkVictory($uid) {
            $list       = DB::asArray("SELECT *, ((SELECT SUM(votes) FROM clt_okvotes_other WHERE game_id=g.id AND `time`<=g.`time` + INTERVAL 1 DAY) + 
                                                  (SELECT SUM(votes) FROM clt_okvotes WHERE game_id=g.id AND `time`<=g.`time` + INTERVAL 1 DAY)) AS votes, g.mlp AS mlp
            ...
            foreach ($list as $key=>$item) {
                if (($item['rate'] == 0) || ($item['votes'] == 0)) { // Отсеиваем все с нулевым рейтингом
                    ...
                    $query = "SELECT *,
                                ((SELECT SUM(votes) FROM clt_okvotes_other WHERE game_id=g.id AND `time`<='{$endTime}') + 
                                 (SELECT SUM(votes) FROM clt_okvotes WHERE game_id=g.id AND `time`<='{$endTime}')) AS votes 
                            FROM `clt_okgame` g 
                    ...
                    if ($vic['votes'] > $item['votes']) { // Это не победа
                        ...
                        DB::query("INSERT `clt_okgameVictory` VALUES ({$item['id']}, $uid, '{$item['time']}', {$item['votes']}, {$item['mlp']})");
        ...
        public function getHeads($params) {
            return DB::asArray("SELECT * FROM `clt_okheads` WHERE `uid`=%s", $params);
        }
        ...
        public function renameGame($params) {
            return array('result'=>DB::query("UPDATE `clt_okgame` SET name='".mysql_escape_string(urldecode($params[1]))."', `stiker`={$params[2]} WHERE id={$params[0]}"));
        } 
        ...
        public function uploadFromGame($params) {
            ...
            $result     = DB::query("INSERT INTO `clt_okgame` (uid, `group`, rate, `name`, `stiker`, time, mlp, options, debug) VALUES ({$params[0]}, {$group}, 0, '".mysql_escape_string(urldecode($params[1]))."', {$params[2]}, '".date('Y-m-d H:i:s')."', {$params[3]}, {$params[5]}, $debug)");
            $id         = DB::lastID();
            
            if ($params[3] == 0) return $this->uploadAsJPG($GLOBALS["HTTP_RAW_POST_DATA"], $id);
            else return $this->uploadAsMLP($GLOBALS["HTTP_RAW_POST_DATA"], $id);
        }
...
*/        
        ...
        public function sendNotify($params) {
            $backDate = date('Y-m-d', strtotime("-3 DAY"));
