...
class templateController extends appController {
    ...
    function viewjs() {
        ...
        if ($tmpl_id = $this->request->getVar('tid', false)) {
            ...
            if (isset($tmpl['name'])) {
                if ($this->asPNG) $this->title = $locale['PHOTOFRAME'].', PNG шаблон для фото ';
                else $this->title = $locale['PHOTOFRAME'].' для фото '; 
                $this->title .= ($tmpl['name']?$tmpl['name']:('№'.$tmpl_id));
            }        
            if (isset($tmpl['desc'])) {
                if ($this->asPNG) $this->description = $locale['PHOTOFRAME'].' для фотошопа, PNG шаблон. ';
                else $this->description =  $locale['PHOTOFRAME'].' для фото, '; 
