...
class userController extends gameBaseController {
    ...
    public function oflog() {
        ...
        if (($pass = $this->getSafeVar('pass')) && ($email = $this->getSafeVar('email'))) {
            if ($result = DB::line("SELECT * FROM ".DBPREF."users u, ".DBPREF."user_options o WHERE u.uid=o.uid AND u.source=o.source AND o.pass='{$pass}' AND u.email='{$email}' AND u.source='of'")) {
    ...
    public function newuser() {
        ...
        if (($name = trim($this->getSafeVar('name'))) &&
            ...
            if (DB::line("SELECT uid FROM ".DBPREF."users WHERE email='{$email}'")) {
                $res['email_exists'] = 1;
            } else {
                if (!($uid = ss::nauid())) 
                ...
                $result = DB::query("INSERT INTO ".DBPREF."users (uid, nauid, source, nick, email, create_date, last_ip) VALUES ($uid, $uid, '$source', '$name', '$email', NOW(), '{$ip}')");
    ...
    public function forgot_success() {
        ...
        if (($code = $this->getVar('code')) && ($pass =  $this->getVar('pass'))) {
            if ($item = DB::line("SELECT f.*, u.* FROM ".DBPREF."forgotpass f INNER JOIN ".DBPREF."users u ON f.email=u.email WHERE f.code='$code'")) {
