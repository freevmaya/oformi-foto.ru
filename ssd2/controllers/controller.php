...
class controller {
    ...
    function getDescription($default='', $limitWords=40) {
        if (!$default) $default = 'Вставь в фоторамку свою фотографию бесплатно. Фотоэффект или фото-коллаж онлайн, прикольные фотографии';
        return $this->description?limitWords(strip_all_tags($this->description), $limitWords):$default;
    }
    
    function getKeywords($default='') {
        if (!$default) $default = 'фоторамка,фото-рамка,прикольное оформление,онлайн,коллаж онлайн,фото-открытка,оформление фотографий';
    ...
    public function cacheKey() {
        return MAINURL.get_class($this).ss::lang();
    }
    ...
    public function getLink() {
        ...
        return $sheme.$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'];
    }
